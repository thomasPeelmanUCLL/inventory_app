...
export function soldItemToDTO(soldItem: SoldItem, includeItem = false): SoldItemDTO {
  ...
  const createdAt = (soldItem as any).getCreatedAt?.() ? (soldItem as any).getCreatedAt().toISOString() : new Date().toISOString();
  const updatedAt = (soldItem as any).getUpdatedAt?.() ? (soldItem as any).getUpdatedAt().toISOString() : null;
...
export function analyticsToDTO(analytics: any): SoldItemAnalyticsDTO {
  return {
    summary: {
      ...
      averageProfit: analytics.summary.totalTransactions > 0
        ? (Number(analytics.summary.totalProfit) / Number(analytics.summary.totalTransactions)).toFixed(2)
        : '0.00',
      profitMargin: Number(analytics.summary.totalSellPrice) > 0
        ? ((Number(analytics.summary.totalProfit) / Number(analytics.summary.totalSellPrice)) * 100).toFixed(2)
        : '0.00',
    },
    topSellingItems: (analytics.topSellingItems ?? []).map((item: any) => ({
      ...
      profitMargin: Number(item.totalSellPrice) > 0
    ...
    paymentMethodBreakdown: {
      cash: {
        ...
        sellPrice: Number(analytics.paymentMethodBreakdown?.cash?.sellPrice ?? 0).toFixed(2),
      ...
      nonCash: {
        buyPrice: Number(analytics.paymentMethodBreakdown?.nonCash?.buyPrice ?? 0).toFixed(2),
        sellPrice: Number(analytics.paymentMethodBreakdown?.nonCash?.sellPrice ?? 0).toFixed(2),
    ...
    priceVariableBreakdown: (analytics.priceVariableBreakdown ?? []).map((pv: any) => ({
      ...
      averageProfit: pv.count > 0
        ? (Number(pv.totalProfit) / Number(pv.count)).toFixed(2)
        : '0.00',
    })),
  };
}
