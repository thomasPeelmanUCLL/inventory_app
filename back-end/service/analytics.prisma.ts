...
export async function getInventoryAnalyticsPrisma({
  ...
  startDate,
  endDate,
}: { inventoryId: number; startDate?: Date; endDate?: Date }) {
  const whereBase = {
    item: { inventoryId },
  ...
  for (const s of lines) {
    ...
    if (s.payedCash) cashTransactions++; else nonCashTransactions++;
    ...
    const pb = bi.priceBreakdown.get(pvKey) || { sellPrice: Number(s.finalSellPrice), priceVariableName: s.priceVariableName ?? null, quantity: 0, totalBuy: 0, totalSell: 0, profit: 0 };
    pb.quantity += qty;
    pb.totalBuy += buy;
    pb.totalSell += sell;
    pb.profit += profit;
    ...
    const bd = byDay.get(day) || { date: day, totalBuyPrice: 0, totalSellPrice: 0, totalProfit: 0, totalQuantity: 0, transactionCount: 0 };
    ...
    const pvAgg = byPriceVar.get(pvKey) || { priceVariableName: s.priceVariableName ?? 'DEFAULT', count: 0, totalBuyPrice: 0, totalSellPrice: 0, totalProfit: 0 };
    pvAgg.count += 1;
  ...
  return {
    ...
    topSellingItems: Array.from(byItem.values()).map((x: any) => ({
      itemId: x.itemId,
      itemName: x.itemName,
    ...
    })).sort((a, b) => b.totalQuantity - a.totalQuantity).slice(0, 25),
    salesByDay: Array.from(byDay.values()).sort((a, b) => a.date.localeCompare(b.date)),
    paymentMethodBreakdown: {
      cash: {
        buyPrice: Number((Array.from(lines).filter(l => l.payedCash).reduce((acc, s) => acc + Number(s.item.buyPrice) * Number(s.quantity), 0)).toFixed(2)),
        sellPrice: Number((Array.from(lines).filter(l => l.payedCash).reduce((acc, s) => acc + Number(s.finalSellPrice) * Number(s.quantity), 0)).toFixed(2)),
        profit: Number((Array.from(lines).filter(l => l.payedCash).reduce((acc, s) => acc + (Number(s.finalSellPrice) - Number(s.item.buyPrice)) * Number(s.quantity), 0)).toFixed(2)),
