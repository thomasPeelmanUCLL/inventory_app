...
function normalizeSold(item: any): SoldItemDTO {
    return {
        ...
        finalSellPrice:
            typeof item.finalSellPrice === 'string'
                ? parseFloat(item.finalSellPrice)
                : Number(item.finalSellPrice),
...
export function useSoldItems() {
    const { data, error, mutate, isLoading } = useSWR<SoldItemDTO[]>(
        ['/soldItems'] as const,
        async () => {
    ...
    const create = async (payload: SoldItemCreate) => {
        ...
        const withDate: SoldItemDTO = {
            ...created,
            soldAt: created.soldAt || new Date().toISOString(),
        };
    ...
    const update = async (id: number, patch: SoldItemPatch) => {
        ...
        await mutate(
            (data || []).map((it) => (it.id === id ? { ...it, ...updated } : it)),
            { revalidate: false },
        );
        return updated;
    };
...
export function useInventorySoldItems(inventoryId: number | null) {
    ...
    const swr = key
        ? useSWR<SoldItemDTO[]>(
              key,
              async () => (await getSoldItemsByInventoryId(inventoryId!)).map(normalizeSold),
    ...
    const update = async (id: number, patch: SoldItemPatch) => {
        const updated = normalizeSold(await updateSoldItem(id, patch as any));
        await mutate(
            (data || []).map((it: SoldItemDTO) => (it.id === id ? { ...it, ...updated } : it)),
            { revalidate: false },
        );
        return updated;
    };
    ...
    const remove = async (id: number) => {
        await deleteSoldItem(id);
        await mutate(
            (data || []).filter((it: SoldItemDTO) => it.id !== id),
            { revalidate: false },
        );
    };
...
export function useItemSoldItems(itemId: number | null) {
    ...
    const swr = key
        ? useSWR<SoldItemDTO[]>(
              key,
              async () => (await getSoldItemsByItemId(itemId!)).map(normalizeSold),
...
export function useSoldItem(soldItemId: number | null) {
    ...
    const swr = key
        ? useSWR<SoldItemDTO | null>(
              key,
              async () => normalizeSold(await getSoldItemById(soldItemId!)),
...
export function useInventoryAnalytics(
    ...
    const swr = key
        ? useSWR<InventoryAnalyticsDTO>(
              key,
              () => getInventoryAnalytics(inventoryId!, startDate, endDate),
              { revalidateOnFocus: false, dedupingInterval: 30000, errorRetryCount: 2 },
          )
        : ({} as any);
    const refresh = async () => {
